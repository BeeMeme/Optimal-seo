...
class install_sample_schema extends \phpbb\db\migration\migration
{
	...
	{
		return $this->db_tools->sql_column_exists($this->table_prefix . 'users', 'user_optimalseo');
	}
	...
	public function update_schema()
	{
		return [
			'add_tables'		=> [
				$this->table_prefix . 'bestpickrs_optimalseo_table'	=> [
					'COLUMNS'		=> [
						'optimalseo_id'			=> ['UINT', null, 'auto_increment'],
						'optimalseo_name'			=> ['VCHAR:255', ''],
					],
					'PRIMARY_KEY'	=> 'optimalseo_id',
				],
			],
			'add_columns'	=> [
				$this->table_prefix . 'users'			=> [
					'user_optimalseo'				=> ['UINT', 0],
				],
			],
		];
	}
